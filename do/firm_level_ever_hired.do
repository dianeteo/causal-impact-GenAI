use firm_quarter_panel_genai_did.dta, clear
format tq %tq
keep if tq >= yq(2022,1)
capture drop relq t_index
gen relq = tq - yq(2023,1)
drop if relq < -4 | relq > 7
gen t_index = tq - yq(2022,1)
local base_t = yq(2022,4) - yq(2022,1)

reghdfe log_postings ib(`base_t').t_index##i.treated, absorb(firm_id tq) vce(cluster firm_id)
...
twoway (rcap ci_upper ci_lower relq, lwidth(medthick)) (scatter coef relq, msize(medium) msymbol(O)), xline(0, lpattern(dash) lwidth(medium)) yline(0, lpattern(dash)) xlabel(-4(1)7, angle(0) labsize(small)) xtitle("Quarters relative to event (0 = 2023Q1)") ytitle("Effect on log postings (treated vs control)") title("Event study: Firm-level DiD (Ever GenAI integrator firms)") legend(off) graphregion(color(white)) bgcolor(white) note("Note: Period -1 (2022Q4) normalized to zero (reference period).")
restore
